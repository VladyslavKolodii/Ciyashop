...
- (id)initWithFrame:(CGRect)frame
{
   ...
   // self.track.contentMode = UIViewContentModeScaleAspectFill;
...
- (UIImage *)trackBackgroundImage
{
    if(_trackBackgroundImage==nil)
    {
        if(IS_PRE_IOS7())
        {
            UIImage* image = [UIImage imageNamed:@"FilterPriceBarBg"];
...
- (UIImage *)lowerHandleImageNormal
{
    if(_lowerHandleImageNormal==nil)
    {
        if(IS_PRE_IOS7())
        {
            UIImage* image = [UIImage imageNamed:@"FIlterPriceSelectedPoint"];
...
- (CGRect)trackRect
{
    ...
    ////Added by vibhuti to remove space between track image and handles
    xLowerValue = xLowerValue - 5.0f;
    xUpperValue = xUpperValue + 5.0f;
    ////
...
- (UIImage*) trackImageForCurrentValues
{
    ...
    {
        UIImage *image=[self trackImage];
        return image;
    }
    else
    {
        UIImage *image=[self trackCrossedOverImage];
        return image;
    }
}
...
-(void)layoutSubviews
{
    ...
    [Util setPrimaryColorImageView:self.lowerHandle];

    self.lowerHandle.highlightedImage = self.lowerHandleImageHighlighted;
    
//    self.lowerHandle.highlightedImage = [self.lowerHandle.highlightedImage imageWithRenderingMode:UIImageRenderingModeAlwaysTemplate];
//    [self.lowerHandle setTintColor:[Util colorWithHexString:[Util getStringData:kPrimaryColor]]];
    ...
    [Util setPrimaryColorImageView:self.upperHandle];

    self.upperHandle.highlightedImage = self.upperHandleImageHighlighted;
    
//    self.upperHandle.highlightedImage = [self.upperHandle.highlightedImage imageWithRenderingMode:UIImageRenderingModeAlwaysTemplate];
//    [self.upperHandle setTintColor:[Util colorWithHexString:[Util getStringData:kPrimaryColor]]];
    [Util setPrimaryColorImageView:self.upperHandle];
